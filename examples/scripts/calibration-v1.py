...
def main(jsonfile):
    ...
    input_checkpoint_file = os.path.abspath(os.path.join(idir, inp["checkpoint"].format(**format_vars)))
    ...
    driver = NCMCProtonDrive(temperature, topology, system, pressure=pressure, perturbations_per_trial=perturbations_per_trial, propagations_per_step=propagations_per_step)
    ...
    if "coordinates" in reporters:
        ...
        simulation.reporters.append(app.DCDReporter(dcd_output_name, freq, enforcePeriodicBox=True))
    ...
    try:
        ...
        # export the context
        create_calibration_checkpoint_file(output_checkpoint_file, driver, simulation.context, simulation.system,
                                           simulation.integrator, topology_element.text, salinator=salinator)
        ncfile.close()
        os.chdir(lastdir)
