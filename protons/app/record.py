...
def netcdf_file(filename, num_titratable_groups, ncmc_steps_per_trial, num_iterations=None,
    ...
    proton_drive.createVariable('initial_states', int, ('iteration',  'group'))
    proton_drive.createVariable('proposed_final_states', int, ('iteration',  'group'))
    proton_drive.createVariable('proposal_work', np.float64, ('iteration', ))
    ...
    ncmc_integrator.createVariable('ncmc_step', int, ('iteration', 'ncmc_step',))
    # Work per ncmc step at each algorithm iteration
    work_per_step = ncmc_integrator.createVariable('work_per_step', np.float64, ('iteration', 'ncmc_step',),zlib=True)
    work_per_step.unit = "unitless"
    ncmc_integrator.createVariable('naccept', int, ('iteration', 'ncmc_step',),zlib=True)
    ncmc_integrator.createVariable('ntrials', int, ('iteration', 'ncmc_step',),zlib=True)
...
def record_drive_data(ncfile, drive, iteration, sync=True):
    ...
    # drive.last_proposal = (initial_titration_states, final_titration_states, work)
    # Index 0 contains the initial titration state of every group
    for titration_group_index, titration_state in enumerate(drive.last_proposal[0]):
        ncfile['ProtonDrive/initial_states'][iteration, titration_group_index] = titration_state
    ...
    for titration_group_index, titration_state in enumerate(drive.last_proposal[1]):
        ncfile['ProtonDrive/proposed_final_states'][iteration, titration_group_index] = titration_state

    # Index 2 contains the NCMC work of the proposal, plus the sum of all weights g_k
    ncfile['ProtonDrive/proposal_work'][iteration] = drive.last_proposal[2]

    # Append new iteration to the NCMC integrator variable group
    for step_index, step in enumerate(drive.ncmc_stats_per_step):
        ncfile['ProtonDrive/NCMCIntegrator/ncmc_step'][iteration, step_index] = step_index
        ncfile['ProtonDrive/NCMCIntegrator/work_per_step'][iteration, step_index] = step[0]
        ncfile['ProtonDrive/NCMCIntegrator/naccept'][iteration, step_index] = int(step[1])
        ncfile['ProtonDrive/NCMCIntegrator/ntrials'][iteration, step_index] = int(step[2])
