...
class TestTitrationReporter(object):
    ...
    def test_reports(self):
        ...
        assert (
            newreporter.ncfile["Protons/Titration"].dimensions["update"].size == 3
        ), "There should be 3 updates recorded."
        assert (
            newreporter.ncfile["Protons/Titration"].dimensions["residue"].size
            == num_titratable
        ), "There should be {} residues recorded.".format(num_titratable)
        assert (
            newreporter.ncfile["Protons/Titration"].dimensions["atom"].size == num_atoms
        ), "There should be {} atoms recorded.".format(num_atoms)
    ...
    def test_reports_with_salt(self):
        ...
        assert (
            newreporter.ncfile["Protons/Titration"].dimensions["ion_site"].size == 1269
    ...
    def test_state_reporting(self):
        ...
        glu_states = np.asarray(
            [1, 4, 4, 1, 0, 2, 0, 2, 4, 3, 2, 1, 0, 2, 0, 0, 0, 0, 0, 0, 1, 2, 1, 3, 1]
        )
        his_states = np.asarray(
            [2, 1, 0, 1, 2, 2, 1, 2, 2, 1, 0, 2, 2, 1, 2, 2, 2, 2, 2, 2, 0, 2, 2, 2, 1]
        )
        ...
        for i in range(len(glu_states)):
            ...
            ), "Glu state not set correctly."
            assert (
                simulation.drive.titrationGroups[1].state_index == his_states[i]
            ), "His state not set correctly."
        ...
        assert np.array_equal(
            glu_states, recorded_glu_states
        ), "Glutamate states are not recorded correctly."
        assert np.array_equal(
            his_states, recorded_his_states
        ), "Histidine states are not recorded correctly."
    ...
    def _verify_atom_status(iteration, group_index, ncfile, simulation):
        ...
        for id, status in zip(atom_ids, atom_status):
            assert (
                status == ncfile["Protons/Titration/atom_status"][iteration, id]
            ), "Residue {} atom status recorded should match the current status.".format(
                group_index
            )
    ...
    def test_system_charge_reporting(self):
        ...
        for i in range(len(glu_states)):
            ...
            for resi, residue in enumerate(simulation.drive.titrationGroups):
                ...
                tot_charge += icharge
                assert (
                    ncfile["Protons/Titration/{}_charge".format(resi)][i] == icharge
                ), "Residue charge is not recorded correctly."
            assert (
